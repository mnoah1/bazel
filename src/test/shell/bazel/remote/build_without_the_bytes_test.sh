...
EOF
  ...
  local -r intermediate_hash="$(printf 'payload\n' | sha256sum | cut -d' ' -f1)"
  ...
  if remote_cas_file_exist "${intermediate_hash}"; then
    fail "Metadata-only producer uploaded its output blob"
  fi
  ...
  if remote_cas_file_exist "${intermediate_hash}"; then
    fail "Rewound metadata-only producer uploaded its output blob"
  fi
